...
Try {
	...
	[string]$appVersion = 'Community Edition 5.7.14'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.0'
	[string]$appScriptDate = '04/12/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-MSI -Action 'Install' -Path 'mysql-installer-community-5.7.14.0.msi' -Parameters '/qn' -PassThru
		...
		Execute-Process -Path "$exeSchTasks" -Parameters "/Delete /TN \MySQL\Installer\ManifestUpdate /F"
		...
		Execute-Process -Path "$envProgramFilesX86\MySQL\bin\mysqld.exe" -Parameters '--install' -WindowStyle 'Hidden' -WaitForMsiExec
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path 'mysql-installer-community-5.7.14.0.msi' -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
